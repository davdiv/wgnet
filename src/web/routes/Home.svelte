<script lang="ts">
	...
	const onPeerSubmit: EventHandler<SubmitEvent, HTMLFormElement> = async (event) => {
		event.preventDefault();
		await addPeer(peerName, $userInfo$.wgnet?.peerDefaultTags ?? []);
	};

	const onTagSubmit: EventHandler<SubmitEvent, HTMLFormElement> = async (event) => {
		event.preventDefault();
		...
		<div class="max-w-md">
			...
			<div class="flex flex-col gap-2" use:listNavigation>
				{#if hasPeerAccess({ tags: $userInfo$.wgnet?.peerDefaultTags ?? [], id: -1 }, PeerAccess.CreateDelete, $userInfo$.wgnet?.peerAccess)}
					<form class="flex gap-2 input input-ghost w-full items-center" onsubmit={onPeerSubmit}>
				...
				{#if $userInfo$.wgnet?.tagsAdmin}
					<form class="flex gap-2 input input-ghost w-full items-center" onsubmit={onTagSubmit}>
